...
//            self.getRestaurantList()
    ...
    func getRestaurantBeacon(machineArray: NSMutableArray) {
        ...
        let newPost: NSDictionary = ["authId": authId, "authKey": authKey, "beaconId": "335C530F-9800-45EA-9801-FA03095A3556", "machine" : machineArray]
                ...
                if httpResponse.statusCode == 200 {
                    do {
                        ...
                        if status == 0 {
                            //성공
                            scanChk = true
                            restaurantId = post["data"]?.objectForKey("restaurantId") as! Double
...
//            customPop.showInView(self.view, withMessage: "Error occurred during communicating with server. Please try again in a few minutes.", animated: true)
    ...
    func getRestaurantList() {
        ...
        switch(searchType) {
        case "NAME":
            newPost = ["authId": authId, "authKey": authKey, "type" : restaurantType, "lastId":0, "size": 50, "searchType" : searchType, "name" : searchName]
            alertMsg = "Sorry, we don't have any listing under this category yet."
            break;
        case "LOCATION":
            newPost = ["authId": authId, "authKey": authKey, "type" : restaurantType, "lastId":0, "size": 50, "searchType" : searchType, "location" : restaurantLocation]
            alertMsg = "Sorry, we don't have any listing under this category yet."
            break;
        case "FOOD_TYPE":
            newPost = ["authId": authId, "authKey": authKey, "type" : restaurantType, "lastId":0, "size": 50, "searchType" : searchType, "foodType" : foodType]
            alertMsg = "Sorry, we don't have any listing under this category yet."
            break;
        case "POPULARITY":
            newPost = ["authId": authId, "authKey": authKey, "type" : restaurantType, "lastId":0, "size": 50, "searchType" : searchType]
            alertMsg = "Sorry, we don't have any listing under this category yet."
            break;
        case "FAVORITES":
            newPost = ["authId": authId, "authKey": authKey, "type" : restaurantType, "lastId":0, "size": 50, "searchType" : searchType]
                    ...
                    do {
                        ...
                        if status == 0 {
                            ...
                            if(chk?.count == 0) {
                                ...
                                restaurantList = data.valueForKey("items") as! NSArray
