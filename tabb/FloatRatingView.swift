...
public class FloatRatingView: UIView {
    ...
    /**
     Set EvaluationId when Evaluate Food & Restaurant Service
     */
    @IBInspectable public var evaluationId: Double = 0
...
//                CGFloat((self.emptyImageViews.count - 1))
            ...
            for i in 0..<self.maxRating {
                let imageFrame = CGRectMake(i==0 ? 0:CGFloat(i)*(imageViewSize.width), 0, imageViewSize.width, imageViewSize.height)
    ...
    override public func touchesEnded(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        if let delegate = self.delegate {
            ...
            delegate.floatRatingView(self, didUpdate: self.rating, evaluationId: self.evaluationId)
        }
    }
